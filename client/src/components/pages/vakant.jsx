...
const Vakant = ({ item }) => {
  return (
    <div className="bg-white shadow-lg rounded-lg h-[40vh] p-4">
      <p className="text-slate-400">{item.createdAt}</p>
      <h1 className="text-2xl font-bold">{item.title}</h1>
      <div className="grid grid-cols-2">
        <div className="flex flex-col items-start justify-start gap-2 my-2 text-green-500">
      ...
      <p className="text-slate-500 my-4">
        Ish haqi: <span className="text-red-600">"Oylik"</span>{" "}
        <span className="text-sky-600">{item.salt}</span>
      </p>
      <p className="text-slate-600">
        {item.description.slice(0, 150)}{" "}
        <span className="text-cyan-600 cursor-pointer">...more</span>{" "}
      </p>
